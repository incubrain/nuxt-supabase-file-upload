...
import { COOKIE_KEYS, COOKIE_OPTIONS, EVENTS } from '@/types/supabase'

let supabase: SupabaseClient

function setCookieValue(key: string, value: string | null) {
...
export default defineNuxtPlugin({
  ...
  async setup() {
    ...
    supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (![EVENTS.SIGNED_IN, EVENTS.TOKEN_REFRESHED].includes(event) || !session)
        return
